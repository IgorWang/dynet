...
int main(int argc, char **argv) {
    ...
    if (argc == 2) {
        ...
        const unsigned HIDDEN_SIZE = 8;//隐藏层的小
        p_W = m.add_parameters({HIDDEN_SIZE, 2}); // p_W shape = (HIDDEN_SIZE * 2)
        p_b = m.add_parameters({HIDDEN_SIZE}); // bias
    ...
    Expression x = input(cg, {2}, &x_values);// 输入的shape为(2,)
    ...
    Expression loss_expr = squared_distance(y_pred, y); // loss function
